...
function ResultsContent() {
    ...
    useEffect(() => {
      const safetyTimeout = setTimeout(() => {
        if (foundationLoading) {
          console.log("Safety timeout triggered - foundation has been loading too long");
          setFoundationLoading(false);
          setError("Loading timed out. Please try again or check console for errors.");
        }
      }, 15000); // 15 seconds
   ...
   const generateCalendar = useCallback(async (submissionData, foundation) => {
    ...
    }, [id, generateFoundation]); // Added generateFoundation to the dependency array
           ...
           <div className="card bg-base-100 shadow-xl">
             <div className="card-body">
              ...
              {foundationLoading ? (
                <div className="text-center p-10">
                  <span className="loading loading-dots loading-lg"></span>
                  <p className="mt-2">Generating foundation... this may take a minute</p>
                </div>
              ) : foundationStrategy ? (
                <MarkdownContent content={foundationStrategy} /> 
              ) : (
