...
// api/profile/save-strategy/route.js
...
//   console.log('API /save-strategy: Starting request');
  
//   const session = await getServerSession(authOptions);
//   if (!session || !session.user?.id) return NextResponse.json({ error: "Not authenticated" }, { status: 401 });
//   const userId = session.user.id;
//   console.log(`API /save-strategy: Processing request for user ${userId}`);

//   try {
//     const { submissionId, answers, foundation, calendar, contentCalendar, overwrite } = await req.json();
//     const isStrategyOnlySave = foundation && (!calendar || !contentCalendar);
//     console.log(`API /save-strategy: Request type - ${isStrategyOnlySave ? 'Strategy only' : 'Calendar save'}, Overwrite: ${overwrite}`);

//     console.log(`API /save-strategy: Connecting to MongoDB - ${Date.now() - startTime}ms`);
//     await connectMongo();
    
//     console.log(`API /save-strategy: Finding user - ${Date.now() - startTime}ms`);
//     const currentUser = await User.findById(userId);
//     if (!currentUser) return NextResponse.json({ error: "User not found" }, { status: 404 });
...
//     // OPTIMIZED: Direct assignment for overwrite
//     if (overwrite === true) {
//       console.log(`API /save-strategy: Clearing all calendars due to overwrite flag - ${Date.now() - startTime}ms`);
//       // Use direct assignment for better performance
//       currentUser.profile.linkedinStrategy.savedCalendars = [];
//     } else if (!Array.isArray(currentUser.profile.linkedinStrategy.savedCalendars)) {
...
//     if (isStrategyOnlySave) {
//       // Handle strategy-only save (from results page)
//       console.log(`API /save-strategy: Processing strategy-only save - ${Date.now() - startTime}ms`);
...
//       currentUser.profile.linkedinStrategy.submissionId = submissionId;
//       currentUser.profile.linkedinStrategy.savedAt = new Date();
//       // savedCalendars was already handled above
      
//       console.log(`API /save-strategy: Saving strategy to DB - ${Date.now() - startTime}ms`);
//       await currentUser.save();
      
//       console.log(`API /save-strategy: Strategy save completed in ${Date.now() - startTime}ms`);
//       return NextResponse.json({ success: true, message: "Strategy foundation saved!" });

//     } else {
//       // Handle calendar save (from profile page)
//       console.log(`API /save-strategy: Processing calendar save - ${Date.now() - startTime}ms`);
...
//       // --- Calculate Start Date ---
//       console.log(`API /save-strategy: Calculating new calendar dates - ${Date.now() - startTime}ms`);
//       let newStartDate;
//       if (savedCalendars.length === 0) {
//         const now = startOfDay(new Date());
//         newStartDate = getDay(now) === 1 ? now : nextMonday(now);
//         console.log(`API /save-strategy: First calendar starting on:`, newStartDate);
//       } else {
//         // Sort ASCENDING to get the last element (latest end date)
//         const calendarsWithDates = savedCalendars
//           .map(cal => ({ 
//             endDate: cal.endDate instanceof Date ? cal.endDate : new Date(cal.endDate) 
//           }))
//           .filter(d => isValid(d.endDate));
          
//         if (calendarsWithDates.length === 0) throw new Error("No valid end dates in existing calendars");
//         calendarsWithDates.sort((a, b) => a.endDate.getTime() - b.endDate.getTime());
//         const latestEndDate = calendarsWithDates[calendarsWithDates.length - 1].endDate;
//         console.log(`API /save-strategy: Latest calendar end date:`, latestEndDate);
...
//             newStartDate = addDays(newStartDate, 7);
//         }
//         console.log(`API /save-strategy: New calendar will start on:`, newStartDate);
//       }

//       // --- Calculate End Date using post dates ---
//       const postsPerWeek = Math.ceil(calendarRows.length / 4); // Assuming 4 weeks
//       const lastPostIndex = calendarRows.length - 1;
//       console.log(`API /save-strategy: Calculating endDate: lastPostIndex=${lastPostIndex}, postsPerWeek=${postsPerWeek}`);
...
//       // Use calculatePostDateBackend to find the date of the LAST post
//       const newEndDate = calculatePostDateBackend(newStartDate, lastPostIndex, postsPerWeek);
//       console.log(`API /save-strategy: Calculated newEndDate:`, newEndDate);
...
//       // Update strategy fields and push new calendar
//       console.log(`API /save-strategy: Updating strategy fields - ${Date.now() - startTime}ms`);
//       currentUser.profile.linkedinStrategy.foundation = foundation;
//       currentUser.profile.linkedinStrategy.answers = answers;
//       currentUser.profile.linkedinStrategy.submissionId = submissionId;
//       currentUser.profile.linkedinStrategy.savedCalendars.push(newCalendarEntry);
...
//       console.log(`API /save-strategy: Saving calendar to DB - ${Date.now() - startTime}ms`);
//       await currentUser.save();
      
//       console.log(`API /save-strategy: Calendar save completed in ${Date.now() - startTime}ms`);
//       return NextResponse.json({ success: true, message: "Calendar saved successfully!" });
//     }
//   } catch (error) {
//     console.error(`API /save-strategy: Error after ${Date.now() - startTime}ms:`, error);
//     return NextResponse.json({ error: error.message, details: error.stack }, { status: 500 });
//   } finally {
//     console.log(`API /save-strategy: Request completed in ${Date.now() - startTime}ms`);
//   }
// }