...
export async function POST(request) {
  try {
    ...
    // Get the data from the request - support both chat data format and form data format
    const requestData = await request.json();
    
    // Check which format we're receiving (chat data or form data)
    const isStrategyData = requestData.strategyData !== undefined;
    let answers;
    
    if (isStrategyData) {
      // We're receiving data from the chat extraction
      const { strategyData } = requestData;
      
      // Clean and transform the chat data to match the format expected by createFoundationPrompt
      answers = {
        industry: strategyData.industry || "Not specified",
        role: strategyData.role || "Not specified",
        primaryGoal: strategyData.linkedinGoals && strategyData.linkedinGoals.length > 0 
          ? strategyData.linkedinGoals[0] 
          : "Not specified",
        targetAudience: strategyData.targetAudience && strategyData.targetAudience.length > 0 
          ? strategyData.targetAudience[0] 
          : "Not specified",
        commercialObjectives: strategyData.desiredResults && strategyData.desiredResults.length > 0 
          ? strategyData.desiredResults[0] 
          : "Not specified",
        audienceChallenges: strategyData.audienceChallenges || [],
        audienceGoals: strategyData.audienceGoals || [],
        uniquePerspective: strategyData.communicationStyle || "Not specified",
        ...
        userVoice: strategyData.communicationStyle || "Not specified" // Reuse if no specific tone provided
      };
    } else {
      // We're receiving form data directly
    ...
    // Format the prompt for foundation
    const prompt = createFoundationPrompt(answers);
